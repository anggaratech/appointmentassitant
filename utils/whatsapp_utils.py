...
import requests
import re
from utils.openai import OpenAIServices
...
async def send_message(data, config):
    headers = {
...
async def process_whatsapp_message(body, config):
    ...
    result_oai= await OpenAIServices().conversation(sentence=message_body, wa_id= wa_id, wa_name=name)
    print(result_oai)
    response = process_text_for_whatsapp(result_oai)
    print(f"+{wa_id}")
    data = get_text_message_input(f"+{wa_id}", response)
    await send_message(data, config)
